...
def init_nem_architecture(key, n_state=5, n_act=5, n_hidden=10):
    key, subkey = random.split(key)
    forward = init_mlp_architecture(
        subkey, n_input=n_act + n_state, n_output=n_act, n_hidden=n_hidden
    )

    key, subkey = random.split(key)
    expand = init_mlp_architecture(subkey, n_input=1, n_output=n_act, n_hidden=n_hidden)

    key, subkey = random.split(key)
    collapse = init_mlp_architecture(subkey, n_input=n_act, n_output=1, n_hidden=n_hidden)

    key, subkey = random.split(key)
    update = init_mlp_architecture(
        subkey, n_input=n_act + n_act + n_state, n_output=n_state, n_hidden=n_hidden
    )

    key, subkey = random.split(key)
    to_prev = init_mlp_architecture(
        subkey, n_input=n_state, n_output=n_state, n_hidden=n_hidden
    )

    key, subkey = random.split(key)
    to_next = init_mlp_architecture(
        subkey, n_input=n_state, n_output=n_state, n_hidden=n_hidden
    )

    key, subkey = random.split(key)
    first_hidden = init_mlp_architecture(
...
def apply_forward_nem(meta, base, x):
    y = jnp.expand_dims(x, -1)
    y = apply_meta_net(meta["expand"], y)
    ...
    for i, (w, h) in enumerate(zip(base["w"], base["h"][1:])):
        ...
        y = apply_meta_net(meta["forward"], y)
    pred = apply_meta_net(meta["collapse"], y).squeeze(-1)
...
def update(meta, base, x, y):
    ...
    for i, w in enumerate(base["w"]):
        ...
        prv = apply_meta_net(meta["to_prev"], prev_state)
        prv = prv / jnp.sqrt(jnp.expand_dims(jnp.sum(prv**2, -1), -1) + 1e-10)
        nxt = apply_meta_net(meta["to_next"], next_state)
